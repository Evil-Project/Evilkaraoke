...
public final class EvilkaraokeCommandService {
    ...
    private static final List<String> ROOT_SUBCOMMANDS = List.of(
            ...
            "setlist", "playlist", "radio", "current", "previous", "queue", "cancel", "pause", "resume",
            "next", "stop", "audience", "stats", "issue"
    );
    ...
    public int execute(CommandActor actor, String label, String[] args) {
        ...
        return switch (subcommand) {
            ...
            case "previous" -> control(actor, "previous");
            case "next" -> control(actor, "next");
            case "queue" -> queue(actor, args, label);
            case "cancel" -> cancel(actor, args);
            case "pause" -> control(actor, "pause");
            case "resume" -> control(actor, "resume");
            case "stop" -> control(actor, "stop");
    ...
    public static List<String> suggest(String[] args) {
        ...
        if (args.length == 2 && "queue".equalsIgnoreCase(args[0])) {
            return filterPrefix(List.of("move", "1"), args[1]);
        }
        ...
        if (args.length == 2 && "cancel".equalsIgnoreCase(args[0])) {
            return filterPrefix(List.of("all"), args[1]);
        }
    ...
    public List<String> suggest(CommandActor actor, String[] args) {
        ...
        if (args.length == 2 && "cancel".equalsIgnoreCase(args[0])) {
            return filterPrefix(cancelArgumentSuggestions(actor), args[1]);
        }
    ...
    private List<String> cancelArgumentSuggestions(CommandActor actor) {
        if (!actor.isPlayer() || !actor.hasPermission("evilkaraoke.command.cancel")) {
            return List.of();
        }
        boolean isAdmin = actor.hasPermission("evilkaraoke.admin.cancel");
    ...
    private List<String> moveFromPositionSuggestions(CommandActor actor) {
        if (!actor.isPlayer() || !actor.hasPermission("evilkaraoke.command.queue")) {
            return List.of();
        }
    ...
    private int help(CommandActor actor, String label) {
        ...
        actor.sendMessage("/" + label + " cancel <position|all> - remove queued song(s)");
        actor.sendMessage("/" + label + " queue move <from> <to> - reorder your queued songs");
        actor.sendMessage("/" + label + " previous|next - navigate tracks");
        actor.sendMessage("/" + label + " pause|resume|stop - controls");
    ...
    private int doctor(CommandActor actor) {
        if (!actor.hasPermission("evilkaraoke.admin.doctor")) {
            return deny(actor);
        }
        ...
        if (actor.isPlayer() && core.clientRegistry().session(actor.playerId()).isEmpty()) {
            actor.sendMessage("You have not completed the Evilkaraoke client handshake; install the client mod to hear audio.");
        }
        return 1;
    }
    ...
    private int reload(CommandActor actor) {
        if (!actor.hasPermission("evilkaraoke.admin.reload")) {
            return deny(actor);
        }
        core.reload();
    ...
    private int randomSong(CommandActor actor) {
        ...
        async(core.coordinator().requestRandom(player), track -> {
            ...
            actor.sendMessage("Queued random song: " + track.title() + " - " + track.artist());
    ...
    private int request(CommandActor actor, String[] args) {
        ...
        if (isSongIdMode(args[1])) {
            ...
            async(core.neurokaraokeClient().song(songId).thenCompose(track -> core.coordinator().request(track, player).thenApply(ignored -> track)),
                    track -> {
                        core.statsService().recordRequest(player.id(), player.name(), track.id(), track.title());
                        actor.sendMessage("Queued: " + track.title() + " - " + track.artist());
                    },
    ...
    private int requestUrl(CommandActor actor, KaraokePlayer player, String rawUrl, String title) {
        ...
        async(future,
                track -> {
                    core.statsService().recordRequest(player.id(), player.name(), track.id(), track.title());
                    actor.sendMessage("Queued URL: " + track.title() + " - " + track.artist());
                },
    ...
    private int current(CommandActor actor) {
        ...
        actor.sendMessage("Now playing: " + track.title() + " - " + track.artist());
        actor.sendMessage("Requested by: " + snapshot.current().requesterName());
        actor.sendMessage("Elapsed: " + DurationFormatter.mmss(snapshot.offset()));
        return 1;
    }
    ...
    private int queue(CommandActor actor, String[] args, String label) {
        ...
        if (args.length >= 2 && "move".equalsIgnoreCase(args[1])) {
            return moveQueue(actor, args);
        }
        KaraokeSession.PlaybackSnapshot snapshot = core.coordinator().snapshot();
        ...
        if (snapshot.current() != null) {
            ...
            actor.sendMessage("Now playing: " + currentTrack.title() + " - " + currentTrack.artist());
            actor.sendMessage("Requested by: " + snapshot.current().requesterName() + " | Elapsed: " + DurationFormatter.mmss(snapshot.offset()));
        }
        ...
        for (int i = start; i < Math.min(queue.size(), start + CHAT_PAGE_SIZE); i++) {
            KaraokeSession.QueuedTrack queued = queue.get(i);
            actor.sendMessage(queueItemMessage(actor, queued, requests, i + 1, label));
        }
        if (queue.isEmpty()) {
    ...
    private int moveQueue(CommandActor actor, String[] args) {
        if (!requirePlayer(actor)) {
            return 1;
        }
        if (args.length < 4) {
        ...
        core.coordinator().moveRequest(from, to).ifPresentOrElse(
                moved -> actor.sendMessage("Moved: " + moved.track().title() + " from " + from + " to " + to + "."),
                () -> actor.sendMessage("Could not move that queued song."));
        return 1;
    }

    private int cancel(CommandActor actor, String[] args) {
        if (!requirePlayer(actor)) {
            return 1;
        }
        if (!actor.hasPermission("evilkaraoke.command.cancel")) {
            return deny(actor);
        }
        if (args.length < 2) {
            actor.sendMessage("Usage: /ek cancel <position|all>");
            actor.sendMessage("Use /ek queue to see song positions.");
            return 1;
        }
        if ("all".equalsIgnoreCase(args[1])) {
            return cancelAll(actor);
        }
        int position;
        try {
        ...
        core.coordinator().cancelAt(position).ifPresentOrElse(
                removed -> actor.sendMessage("Removed from queue: " + removed.track().title() + " - " + removed.track().artist()),
    ...
    private int control(CommandActor actor, String action) {
        String permissionAction = switch (action) {
            case "next", "previous" -> "skip";
            default -> action;
        };
        if (!actor.hasPermission("evilkaraoke.playback." + permissionAction)) {
            return deny(actor);
        }
        switch (action) {
    ...
    private int audience(CommandActor actor, String[] args) {
        if (!actor.hasPermission("evilkaraoke.playback.audience")) {
            return deny(actor);
        }
        if (args.length < 2) {
    ...
    private int issue(CommandActor actor) {
        ...
        actor.sendMessage("4. If audio stalls, /ek next to advance the queue.");
        return 1;
    }
    ...
    private void maybeWarnClient(CommandActor actor) {
        if (actor.isPlayer() && core.config().requireClientMod() && core.clientRegistry().session(actor.playerId()).isEmpty()) {
            actor.sendMessage("Heads up: install the Evilkaraoke client mod to actually hear playback.");
        }
    }
    ...
    private CommandMessage queueItemMessage(CommandActor actor, KaraokeSession.QueuedTrack queued, List<KaraokeSession.QueuedTrack> requests, int position, String label) {
        CommandMessage.Builder message = CommandMessage.builder()
                .append(position + ". " + queued.track().title() + " - " + queued.track().artist() + " (by " + queued.requesterName() + ") ");
        ...
        if (requestIndex >= 0 && requestIndex < requests.size()) {
            if (requestIndex > 0 && canMoveRequestedRange(actor, requests, requestIndex, requestIndex - 1)) {
                message.action("[Up]", "/" + label + " queue move " + position + " " + (position - 1), "Move earlier");
                message.append(" ");
            }
            if (requestIndex + 1 < requests.size() && canMoveRequestedRange(actor, requests, requestIndex, requestIndex + 1)) {
                message.action("[Down]", "/" + label + " queue move " + position + " " + (position + 1), "Move later");
                message.append(" ");
            }
        }
        message.action("[Cancel]", "/" + label + " cancel " + position, "Remove this song from the queue");
        return message.build();
    }
    ...
    private static boolean canMoveRequestedRange(CommandActor actor, List<KaraokeSession.QueuedTrack> requests, int fromIndex, int toIndex) {
        if (actor.hasPermission("evilkaraoke.admin.queue")) {
            return true;
        }
    ...
    private int parseRawInt(String value, int fallback) {
        try {
