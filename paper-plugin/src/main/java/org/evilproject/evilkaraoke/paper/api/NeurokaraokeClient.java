...
public final class NeurokaraokeClient {
                ...
                .<CompletableFuture<String>>handle((response, ex) -> {
                    ...
                    if (response.statusCode() >= 400 && response.statusCode() < 500) {
                        return CompletableFuture.failedFuture(
                                new NeurokaraokeApiException(response.statusCode(), response.body()));
                    }
    ...
    private KaraokeTrack trackFromElement(JsonElement element) {
        ...
        if (artist == null) {
            artist = firstStringArray(object, "originalArtists");
        }
        String coverArtists = firstStringArray(object, "coverArtists", "coveredBy");
        ...
        return new KaraokeTrack(id == null ? title : id, TrackType.SONG, title == null ? "Unknown Song" : title, artist, coverArtists, resolved, fallback, duration(object));
    }
